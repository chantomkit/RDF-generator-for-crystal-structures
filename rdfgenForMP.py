...
for material_id in inputs:
    ...
    line = 0
    fileNumber += 1
    print(f"Processing {fileNumber} / {len(inputs)} files.")
    ...
    n = int(np.sum(atom_number))
    print(f"Number of atoms: {n}")
    # if n > 100:
    #     print(f"Processed {fileNumber} / {len(inputs)} files. (Bypassed: n too large)")
    #     fileNotUsed += 1
    #     continue
    lattice_vector = lattice_vector.reshape((3,3))
    r = r.reshape((line - 8, 3))
    ...
    if np.linalg.norm(count) == 0:
        # print(count)
        print(f"Processed {fileNumber} / {len(inputs)} files. (Bypassed: count = 0)")
        fileNotUsed += 1
        continue
    # RDF formula
    for x in range(binmax):
    ...
    if np.linalg.norm(g) == 0:
        # print(g)
        print(f"Processed {fileNumber} / {len(inputs)} files. (Bypassed: g = 0)")
        fileNotUsed += 1
        continue
    g = g / np.sum(g)
    # print(np.sum(g))
    ...
    print(f"Processed {fileNumber} / {len(inputs)} files.")
...
np.save("training_data_4.npy", training_data)
spgrp_dis = np.zeros(230, dtype=int)
for rdf, sys, spgrp in training_data:
    spgrp_dis[int(np.argmax(spgrp))] += 1
print(spgrp_dis)
